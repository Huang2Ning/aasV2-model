...
public abstract class EntityBuilder<T extends Entity, B extends EntityBuilder<T, B>> extends ExtendableBuilder<T, B> {
	...
	*/
	public B entityTypes(List<EntityType> entityTypes) {
		getBuildingInstance().setEntityTypes(entityTypes);
		return getSelf();
	}

	/**
	* This function allows adding a value to the List entityTypes
	* @param entityType desired value to be added
	* @return Builder object with new value for entityTypes
	*/
	public B entityType(EntityType entityType) {
		getBuildingInstance().getEntityTypes().add(entityType);
		return getSelf();
	}
