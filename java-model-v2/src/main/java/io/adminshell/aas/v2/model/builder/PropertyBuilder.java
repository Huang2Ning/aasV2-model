...
public abstract class PropertyBuilder<T extends Property, B extends PropertyBuilder<T, B>> extends ExtendableBuilder<T, B> {
	...
	/**
	* This function allows setting a value for propertyValueIds
	* @param propertyValueIds desired value to be set
	* @return Builder object with new value for propertyValueIds
	*/
	public B propertyValueIds(List<Reference> propertyValueIds) {
		getBuildingInstance().setPropertyValueIds(propertyValueIds);
		return getSelf();
	}

	/**
	* This function allows adding a value to the List propertyValueIds
	* @param propertyValueId desired value to be added
	* @return Builder object with new value for propertyValueIds
	*/
	public B propertyValueId(Reference propertyValueId) {
		getBuildingInstance().getPropertyValueIds().add(propertyValueId);
		return getSelf();
	}
