...
public abstract class RangeBuilder<T extends Range, B extends RangeBuilder<T, B>> extends ExtendableBuilder<T, B> {
	...
	*/
	public B maxs(List<String> maxs) {
		getBuildingInstance().setMaxs(maxs);
		return getSelf();
	}

	/**
	* This function allows adding a value to the List maxs
	* @param max desired value to be added
	* @return Builder object with new value for maxs
	*/
	public B max(String max) {
		getBuildingInstance().getMaxs().add(max);
		return getSelf();
	}
	...
	*/
	public B mins(List<String> mins) {
		getBuildingInstance().setMins(mins);
		return getSelf();
	}

	/**
	* This function allows adding a value to the List mins
	* @param min desired value to be added
	* @return Builder object with new value for mins
	*/
	public B min(String min) {
		getBuildingInstance().getMins().add(min);
		return getSelf();
	}
