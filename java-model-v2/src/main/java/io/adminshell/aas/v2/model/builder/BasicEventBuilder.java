...
public abstract class BasicEventBuilder<T extends BasicEvent, B extends BasicEventBuilder<T, B>> extends ExtendableBuilder<T, B> {
	...
	/**
	* This function allows setting a value for observeds
	* @param observeds desired value to be set
	* @return Builder object with new value for observeds
	*/
	public B observeds(List<Reference> observeds) {
		getBuildingInstance().setObserveds(observeds);
		return getSelf();
	}

	/**
	* This function allows adding a value to the List observeds
	* @param observed desired value to be added
	* @return Builder object with new value for observeds
	*/
	public B observed(Reference observed) {
		getBuildingInstance().getObserveds().add(observed);
		return getSelf();
	}
